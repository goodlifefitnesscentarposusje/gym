...
// FIX: Funkcija za formatiranje termina
function formatTermin(termin) {
  ...
  return termin.toString().trim();
}

// FIX: Funkcija za formatiranje telefona
function formatTelefon(telefon) {
  if (!telefon || telefon === '#ERROR!' || telefon === '' || telefon === null) {
    return 'N/A';
  }
  return telefon.toString().trim();
}
    ...
    if (data.action === 'cancel') {
      ...
      for (let i = 1; i < vals.length; i++) {
        ...
        if (email===data.email && code===data.code && status.startsWith('potvrđeno')){
          ...
          const telefon = formatTelefon(r[3]); // D = Telefon - FIXED
          const datum = FMT(r[4]); // E = Datum
          const termin = formatTermin(r[5]); // F = Termin - FIXED
          ...
          // FIX: Ukloni CC iz korisničkog maila - možda blokira slanje
